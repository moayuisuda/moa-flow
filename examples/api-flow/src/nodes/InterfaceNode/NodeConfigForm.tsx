    ...
    return (
      <Form<FlowNodeConfig> form={form} >
                    ...
                    return prev.map((item) => {
                      if (item.value === curr.namespace) {
                        item.children.push({
                          ...curr,
                          value: curr.method,
                          label: `${curr.method}(${curr.description?.split("@param")?.[0]
                            })`,
                        });
                      }
                      return item;
        ...
        <Form.Item noStyle shouldUpdate>
          {({ getFieldsValue }) => {
            const { interface: selectInterface, inputParams = [] } = getFieldsValue();
            if (!interfaceSchema || !selectInterface?.length) {
              return null;
            }
            const inputParamsSchema: IParam[] = getInterfaceByName({ schema: interfaceSchema, name: selectInterface?.join('.'), })?.inputParams || [];
            const generateDataSource: any = (inputParam: IParam, valueLine: IParamFront, parentPath: string[]) => {
              const children: IParamFront[] | undefined = ['object', 'array'].includes(inputParam.definition?.type) ? Object.keys(inputParam.definition?.properties || {}).map(childPropertyName => {
                const childProperty = inputParam.definition?.properties?.[childPropertyName];
                return generateDataSource(childProperty, valueLine?.children?.find(item => item.id === childProperty.id) as IParamFront, [...parentPath, inputParam.id]);
              }) : undefined;
              return {
                id: inputParam.id,
                ...
                type: inputParam.type || (inputParam?.schema?.$ref?.split('/') || []).pop() || inputParam?.schema?.type || '',
            ...
            const dataSource = inputParamsSchema.map((item) => {
              return generateDataSource(item, inputParams.find(({ id }: IParamFront) => id === item.id), []);
            });
            return (
